...
from matching_metrics import hybrid_metric, minutiae_metric, matching, calculate_true_matches, average_minutiae_match_distance, cross_correlations
...
from utils.visualization import send_to_visualize
...
def run_inference(
	...
	with torch.inference_mode():
		for idx, (eval, orig) in enumerate(zip(loader1, loader2)):
			...
			Sb, control_points = alignment(Sa, Sb)
			Sb_aligned = warp_original(orig_Sb, control_points)
			
			corr_map = cross_correlations(Sa, Sb)
			send_to_visualize(Sb1, Sb, idx)
			
			average_correlation = corr_map.mean().item()
			all_correlations.append(average_correlation)

			print(f"Average Cross Correlation: {average_correlation}")
	print("Inference completed.")
	print("Calculating performance...")

	average_correlation = sum(all_correlations) / len(all_correlations)
	print(f"Average Cross Correlation: {average_correlation:.4f}")
