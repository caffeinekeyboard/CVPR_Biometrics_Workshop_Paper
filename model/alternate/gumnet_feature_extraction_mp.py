...
# Assuming FeatureL2Norm is correctly implemented in your repository
...
class GumNetFeatureExtractionMP(nn.Module):
    ...
    Architectural Flow & Tensor Dimensions:
        ...
        - Block 1:
            - Conv2d:         (B, 32, 190, 190)
            - MaxPool2d:      (B, 32, 95, 95)
        - Block 2:
            - Conv2d:         (B, 64, 93, 93)
        - Block 3:
            - Conv2d:         (B, 128, 91, 91)
            - MaxPool2d:      (B, 128, 45, 45)
        - Block 4:
            - Conv2d:         (B, 256, 43, 43)
            - MaxPool2d:      (B, 256, 21, 21)
        - Block 5:
            - Conv2d:         (B, 512, 19, 19)
            - AdaptiveMaxPool:(B, 512, 14, 14)   | Reduces spatial dimensions gracefully
            - L2Norm:         (B, 512, 14, 14)   | Normalized across dim=1 (channels)

    Examples:
        >>> feature_extractor = GumNetFeatureExtractionMP(in_channels=1)
    ...
    def __init__(self, in_channels=1):
        ...
        # --- NEW: Adaptive Max Pooling to enforce exact output dimensions ---
        self.adaptive_pool = nn.AdaptiveMaxPool2d((14, 14))
    ...
    def forward(self, x, branch):
        ...
        """
        if branch not in ['Sa', 'Sb']:
            raise ValueError("Branch must be specified as 'Sa' or 'Sb'.")
            
        # --- BLOCK 1 ---
        x = self.shared_conv1(x)               # [B, 32, 190, 190]
        x = F.relu(x)
        x = self.bn1_sa(x) if branch == 'Sa' else self.bn1_sb(x)
        x = self.pool1(x)                      # [B, 32, 95, 95]
        
        # --- BLOCK 2 ---
        x = self.shared_conv2(x)               # [B, 64, 93, 93]
        x = F.relu(x)
        x = self.bn2_sa(x) if branch == 'Sa' else self.bn2_sb(x)
        
        # --- BLOCK 3 ---
        x = self.shared_conv3(x)               # [B, 128, 91, 91]
        x = F.relu(x)
        x = self.bn3_sa(x) if branch == 'Sa' else self.bn3_sb(x)
        x = self.pool3(x)                      # [B, 128, 45, 45]
        
        # --- BLOCK 4 ---
        x = self.shared_conv4(x)               # [B, 256, 43, 43]
        x = F.relu(x)
        x = self.bn4_sa(x) if branch == 'Sa' else self.bn4_sb(x)
        x = self.pool4(x)                      # [B, 256, 21, 21]
        
        # --- BLOCK 5 ---
        x = self.shared_conv5(x)               # [B, 512, 19, 19]
        x = self.bn5_sa(x) if branch == 'Sa' else self.bn5_sb(x)
        
        # --- ADAPTIVE POOLING & NORMALIZATION ---
